...
//function perhtiungan nilai dari gap untuk menentukan bobot yang akan di lakukan perhitungan dengan menggunakan
 ...
 ?> </p>
    <div class="table-responsive">
		...
		$query_rs_bobot =  sprintf("SELECT nilai_bobot, faktor_id FROM tb_bobot INNER JOIN tb_kriteria ON kriteria_id = id_kriteria WHERE alternatif_id = %s AND kriteria_id = %s", 
										GetSQLValueString($koneksi, $row_rs_alternatif2['id_alternatif'], "float"),
										GetSQLValueString($koneksi, $row_rs_kriteria['id_kriteria'], "float"));
		$rs_bobot = mysqli_query($koneksi, $query_rs_bobot) or die(mysqli_error($koneksi));
		$row_rs_bobot = mysqli_fetch_assoc($rs_bobot);
		$totalRows_rs_bobot = mysqli_num_rows($rs_bobot);
    ...
    <div class="row">
                ...
                <table width="100%" class="table table-striped table-bordered">
                    <thead>
                        <tr bgcolor="#003366">
                            ...
                            <th><span class="style1">NAMA ALTERNATIF</span></th>
                            <th>
                                <div align="center" class="style1">CF</div>
                            </th>
                            <th>
                                <div align="center" class="style1">SF</div>
                            </th>
                            <th>
                                <div align="center" class="style1">NILAI AKHIR</div>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
	   ...
	   foreach($rs_kriteria as $row_rs_kriteria) {
		...
		mysqli_select_db($koneksi, $database_koneksi);
		$query_rs_bobot =  sprintf("SELECT nilai_bobot, faktor_id FROM tb_bobot INNER JOIN tb_kriteria ON kriteria_id = id_kriteria WHERE alternatif_id = %s AND kriteria_id = %s", 
										GetSQLValueString($koneksi, $row_rs_alternatif3['id_alternatif'], "float"),
										GetSQLValueString($koneksi, $row_rs_kriteria['id_kriteria'], "float"));
		$rs_bobot = mysqli_query($koneksi, $query_rs_bobot) or die(mysqli_error($koneksi));
		$row_rs_bobot = mysqli_fetch_assoc($rs_bobot);
		$totalRows_rs_bobot = mysqli_num_rows($rs_bobot);
	 ...
	 echo $sf = $total / $faktorku[1];
	  ?> </td>
		...
		echo $hasil;
	  ?>
                                <input type="hidden" name="name<?= $no;?>"
                                    value="<?php echo number_format($hasil,2); ?>" />
                                <input type="hidden" name="id_tempbobot<?= $no; ?>" value="<?php echo $row_rs_alternatif3['id_alternatif']; ?>" />
                            </td>
                        </tr>
                        <?php 
	$no++;
