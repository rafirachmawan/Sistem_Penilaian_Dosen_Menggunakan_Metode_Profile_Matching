...
if (isset($_GET['search'])) {
  $colname_rs_historylogin = $_GET['search'];
	 ...
	 $query_rs_historylogin =  sprintf("SELECT username_login, status_login, added_login, Nama FROM history_login LEFT JOIN vw_login ON username_login = Login WHERE Nama LIKE %s OR username_login LIKE %s", GetSQLValueString($koneksi, "%" . $colname_rs_historylogin . "%", "text"),  GetSQLValueString($koneksi, "%" . $colname_rs_historylogin . "%", "text"));
...
$rs_historylogin = mysqli_query($koneksi, $query_limit_rs_historylogin) or die(mysqli_error($koneksi));
$row_rs_historylogin = mysqli_fetch_assoc($rs_historylogin);
 

if (isset($_GET['totalRows_rs_historylogin'])) {
  $totalRows_rs_historylogin = $_GET['totalRows_rs_historylogin'];
} else {
  $all_rs_historylogin = mysqli_query($koneksi, $query_rs_historylogin);
  $totalRows_rs_historylogin = mysqli_num_rows($all_rs_historylogin);
}
...
<thead>
  ...
  <tr>
    ...
    <?php 
	$no++;
	} while ($row_rs_historylogin = mysqli_fetch_assoc($rs_historylogin)); ?>
    </tbody>
</table>
<!-- /.box-body -->
